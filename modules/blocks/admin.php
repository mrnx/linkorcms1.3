...
function AdminBlocksMain()
{
	...
	.'<tr>
		<th style="background-repeat: no-repeat;">Заголовок</th>
		<th>Положение</th>
		<th>Тип</th>
		<th>Кто видит</th>
		<th>Статус</th>
...
function AdminBlocksEdit( $a )
{
	...
	if(isset($_POST['type']) || $a == 'edit'){
		...
		unset($r);
		FormRow('Заголовок', $site->Edit('title', $b_title, false, 'style="width:200px;"'));
...
function AdminBlocksSave( $a )
{
	...
	if($a == 'newsave'){
		...
		if($block[0]['position'] != $_POST['position'][0]){ // Пересчитываем все order в предыдущем position
