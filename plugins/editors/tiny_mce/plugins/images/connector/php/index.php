...
//				}
			...
			case 'showdir':
				...
				echo $this->ShowDir($path, $_POST['pathtype']); // "", "images" || "files"
				exit();
				break;
	...
	function DirStructure($type, $innerDirs = 'first', $currentDir = '', $level = 0)
	{
		...
		if ($innerDirs == 'first') {
			...
			foreach ($innerDirs as $v) {
				$ret = '<div class="folder' . ucfirst($type) . ' ' . $firstAct . '" path="" pathtype="' . $type . '">' . ($type == 'images'
				  ? 'Изображения' : 'Файлы') . ($v['count'] > 0 ? ' (' . $v['count'] . ')'
		...
		foreach ($innerDirs as $v) {
			...
			if (isset($v['childs'])) {
				...
				$ret .= '<div class="' . $folderClass . ' ' . $folderAct . '" path="' . $v['path'] . '" title="' . $files . '" pathtype="' . $type . '">' . $v['name'] . ($v['count'] > 0
				  ? ' (' . $v['count'] . ')' : '') . '</div><div class="folderOpenSection" ' . $folderOpen . '>';
				...
				$ret .= '<div class="folderClosed ' . $folderAct . '" path="' . $v['path'] . '" title="' . $files . '" pathtype="' . $type . '">' . $v['name'] . ($v['count'] > 0
				  ? ' (' . $v['count'] . ')' : '') . '</div>';
			}
		}
	...
	function DirPath($type, $path = '')
	{
		...
		$ret = '<div class="addrItem" path="" pathtype="' . $type . '" title=""><img src="img/' . ($type == 'images'
		  ? 'folder_open_image'
		  : 'folder_open_document') . '.png" width="16" height="16" alt="Корневая директория" /></div>';
		$i = 0;
		$addPath = '';
		if (is_array($path)) {
			foreach ($path as $v) {
   ...
   <table class="imageBlock0" cellpadding="0" cellspacing="0" filename="' . $v['general']['filename'] . '" fname="' . $v['general']['name'] . '" type="' . $type . '" ext="' . strtoupper($v['general']['ext']) . '" path="' . $v['general']['path'] . '" linkto="' . $v['general']['link'] . '" fsize="' . $v['general']['size'] . '" fsizetext="' . $this->bytes_to_str($v['general']['size']) . '" date="' . date('d.m.Y H:i', $v['general']['date']) . '" fwidth="' . $v['general']['width'] . '" fheight="' . $v['general']['height'] . '" md5="' . $v['general']['md5'] . '" ' . $middle_thumb_attr . '><tr><td valign="bottom" align="center">
	...
	{
		// "", "images" || "files"
		$dir = $this->AccessDir($dir, $type); // "/home/medvitro/public_html/uploads/storage"
		if (!$dir) return false;
		//set_time_limit(120);
